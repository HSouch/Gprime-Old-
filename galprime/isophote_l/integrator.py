...
class _NearestNeighborIntegrator(_Integrator):
    def integrate(self, radius, phi):
        ...
        i = int(radius * np.cos(phi + self._geometry.pa)
                + self._geometry.x0)
        j = int(radius * np.sin(phi + self._geometry.pa)
                + self._geometry.y0)
...
class _BiLinearIntegrator(_Integrator):
    def integrate(self, radius, phi):
        ...
        # Get image coordinates of (radius, phi) pixel
        x_ = radius * np.cos(phi + self._geometry.pa) + self._geometry.x0
        y_ = radius * np.sin(phi + self._geometry.pa) + self._geometry.y0
        i = int(x_)
        j = int(y_)
        fx = x_ - i
        fy = y_ - j
